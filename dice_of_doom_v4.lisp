...
(load (relative->absolute "dice-of-doom-web.lisp"))


;;; プレイ人数を4人にする
(defparameter *num-players* 4)
(defparameter *die-colors*
  ;; 4人のプレイヤーそれぞれのサイコロの色 (R G B)
  '((255 63 63) (63 63 255) (63 255 63) (255 63 255)))
...
(defparemeter *dice-probability* #(#(0.84 0.97 1.0 1.0)
                                   #(0.44 0.78 0.94 0.99)
                                   #(0.15 0.45 0.74 0.91)
                                   #(0.04 0.19 0.46 0.72)
                                   #(0.01 0.06 0.22 0.46)))
               ...
               (lambda (move)
                 (let ((path (car move)))
                   (if (path
                        (let* ((src (car path))
                               (dst (cadr path))
                               (probability (aref *dice-probability*
                                                  (1- (dice dst)))
                                            (- (dice src) 2))))
