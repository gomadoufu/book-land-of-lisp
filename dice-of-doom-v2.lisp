...
(load (relative->absolute "dice-of-doom.lisp"))
(load (relative->absolute "lazy.lisp"))
  ...
  (let* ((hex (aref board pos))
    ...
    (loop for n in (neighbors pos)
          do (let* ((nhex (aref board n))
                    (nplayer (car nhex))
                    (ndice (cadr nhex)))
               (when (and (not (eq nplayer player))
                          (> dice ndice))
                 (return t))))))
...
(defun ab-rate-position (tree player upper-limit lower-limit)
  (let ((moves (caddr tree)))
    (if (not (lazy-null moves))
        (if (eq (cadr tree) player)
            (apply #'max
                   (ab-get-ratings-max tree player upper-limit lower-limit))
            (apply #'min
                   (ab-get-ratings-min tree player upper-limit lower-limit)))
        (score-board (cadr tree) player))))
